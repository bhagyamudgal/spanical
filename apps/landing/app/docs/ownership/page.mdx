export const metadata = {
    title: "ownership",
    description:
        "Show surviving-line ownership and bus-factor risk from git blame at HEAD.",
};
...
| Option           | Values | Notes                                                                                                                 |
| ---------------- | ------ | --------------------------------------------------------------------------------------------------------------------- |
| All global flags |        | See [global flags](/docs/global-flags); window flags affect only extraction scope, since ownership is a HEAD property |
...
1. Per-file ownership: Path (`repo/path`), Lines, Primary owner (the author
   over 50% of surviving lines; `- ` when none), Ownership %, number of
   owners, Sole-owned yes/no. Sole-owned means one author holds at least
   `busFactorThreshold` (default 80%)
2. The bus-factor map: sole-owner files grouped by immediate parent
   directory, so you can see which areas depend on fewest heads. When nothing
   qualifies: `No bus-factor warnings: no file met the 80% sole-ownership threshold.`
   (threshold from config)

A fixed caveat always follows: ownership credits every surviving line to its
single git blame author; Co-authored-by trailers are not split, unlike churn
attribution.
