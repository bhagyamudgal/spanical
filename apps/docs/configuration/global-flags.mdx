---
title: "Global flags"
description: "The one flag set every spanical subcommand accepts, with exact defaults, precedence rules, and the flags some commands reject."
---

Every subcommand accepts the same shared flag set. The table below is the complete list; descriptions are the exact `--help` strings.

| Flag         | Type                                          | Description                                        | Default                                                 |
| ------------ | --------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------- |
| `--config`   | string                                        | Path to spanical.config.ts                         | discovered ([config file](/configuration/config-file))  |
...
| `--period`   | `week` \| `month` \| `quarter`                | Force granularity                                  | automatic ([time windows](/configuration/time-windows)) |
...
brocli also adds `-h, --help` and `-v, --version`. `spanical --version` prints the package version (currently 0.1.0).
