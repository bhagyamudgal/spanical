---
title: "Report"
description: "Generate an engineering insights report: one run produces a terminal headline and a full Markdown artifact."
---
...
| Option           | Values                              | Notes                                                                                                   |
| ---------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------- |
| All global flags |                                     | See [global flags](/configuration/global-flags)                                                         |
...
With no eligible files, the hotspot section prints the same empty-window message documented on [hotspots](/commands/hotspots). The `50` interpolates from config.hotspot.minFileLines. A repository with no commit at or before the window end is skipped with its own note rather than counted as zero activity.
