...
| -------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| `report`       | Headline summary plus a full Markdown artifact, or an offline HTML dashboard with `--format html` | [report](https://spanical.com/docs/report)             |
| `churn`        | Commit volume and throughput churn per period, or per dev with `--by dev`                         | [churn](https://spanical.com/docs/churn)               |
| `contributors` | Per-dev activity across the whole window, including rework lines                                  | [contributors](https://spanical.com/docs/contributors) |
| `size`         | Monthly code size and complexity trend from scc snapshots                                         | [size](https://spanical.com/docs/size)                 |
| `ownership`    | Surviving-line ownership per file and the bus-factor map, from git blame at HEAD                  | [ownership](https://spanical.com/docs/ownership)       |
| `hotspots`     | Files ranked by change frequency times complexity                                                 | [hotspots](https://spanical.com/docs/hotspots)         |
| `tickets`      | Per-dev pull request and issue flow, cycle time, thrash (GitHub)                                  | [tickets](https://spanical.com/docs/tickets)           |
| `reviews`      | Review load, review latency, and team review coverage (GitHub)                                    | [reviews](https://spanical.com/docs/reviews)           |
| `timeline`     | Period-by-period narrative with auto-detected events                                              | [timeline](https://spanical.com/docs/timeline)         |
| `cache`        | Inspect and manage the local cache (`stats`, `rebuild`, `clear`)                                  | [cache](https://spanical.com/docs/cache)               |
| `update`       | Self-update a release install to the latest GitHub release                                        | [update](https://spanical.com/docs/update)             |
...
The `authors` block is the identity bridge: it resolves git emails and GitHub logins to one canonical author so both layers credit the same person once instead of twice. Full option reference at [spanical.com/docs/configuration](https://spanical.com/docs/configuration).

## Time windows

Exactly one selector per run: `--last 30d|6m|1y`, `--this week|month|quarter|year`, `--ytd`, or `--since 2026-01-01 --until 2026-06-30`. With no selector you get the last 12 months. All boundaries are computed in the active timezone (`--tz`, default UTC, weeks start Monday), and period granularity picks itself: weekly up to 8 weeks, monthly up to 18 months, quarterly beyond that. Details at [spanical.com/docs/time-windows](https://spanical.com/docs/time-windows).

## Development

Turborepo monorepo: `packages/cli` holds the CLI, `apps/landing` the Next.js landing and docs site, with shared configs in `packages/typescript-config` and `packages/eslint-config`.
...
Full command reference lives at [spanical.com/docs](https://spanical.com/docs).

The docs site source is `apps/landing`. To work on it locally:
